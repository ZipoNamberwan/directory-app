<div class="modal-content">
    <div class="modal-header">
        <h5 wire:target="showDialog" wire:loading.remove class="modal-title" id="statusDialogLabel">
            {{-- {{ $type != null ? ($type == 'export' ? 'Status Generate Template' : 'Status Upload Template') : '' }} --}}
            Status
        </h5>
        <button wire:click="showDialog('{{ $type }}')" class="m-0 btn btn-icon btn-2 btn-success btn-sm py-1 px-2"
            type="button">
    ...
    <div class="modal-body">
        ...
        <div wire:loading.remove class="table-responsive p-0">
                                    ...
                                    <td class="align-center">
                                        @if ($item->status == 'success')
                                            <button wire:click="downloadExport('{{ $item->id }}')"
                                                class="m-0 btn btn-icon btn-2 btn-outline-secondary btn-sm py-1 px-2"
                                                type="button">
